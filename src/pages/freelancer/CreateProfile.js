...
export default function CreateProfile() {
  ...
  const handleAdd = (e) => {
    e.preventDefault();
    newSkill =
      newSkill.charAt(0).toUpperCase() + newSkill.slice(1).toLowerCase();
  ...
  const submitHandler = async (e) => {
    ...
    await axios.put(
      "http://localhost:8001/api/users/profile/update/",
      {
        skills: skills,
        about: bio,
        offered_service: category,
        offered_service_type: subCategory,
      },
      config
    );
          ...
          {skills.map((skill) => (
            <li key={skill}>
              <p className="bg-light">
                {skill}{" "}
                <em className="btn" onClick={() => deleteSkill(skill)}>
                  X
                </em>
              </p>
            </li>
          ...
          <Form.Select onChange={(e) => setSubCategory(e.target.value)}>
            ...
            {category === "Digital" && (
              <>
                ...
                <option value="Desktop Applicatoin Development">
                  Desktop Applicatoin Development
                </option>
